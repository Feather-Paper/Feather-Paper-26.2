...
public abstract class ConfigModules extends FeatherConfig {

    private static final Set<ConfigModules> MODULES = new HashSet<>();

    public FeatherGlobalConfig config;

    public ConfigModules() {
        this.config = FeatherConfig.config();
    }
    ...
    public static void initModules() throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        Class<?>[] classes = FeatherConfig.getClasses(FeatherConfig.I_CONFIG_PKG).toArray(new Class[0]);
        ...
        if (!enabledExperimentalModules.isEmpty()) {
            FeatherConfig.LOGGER.warn("You have following experimental module(s) enabled: {}, please proceed with caution!", formatModules(enabledExperimentalModules));
        }

        if (!deprecatedModules.isEmpty()) {
            FeatherConfig.LOGGER.warn("The following enabled module(s) has been deprecated: {}, please proceed with caution!", formatModules(deprecatedModules));
        }
    }
    ...
    public static void loadAfterBootstrap() {
        ...
        // Save config to disk
        try {
            FeatherConfig.config().saveConfig();
        } catch (Exception e) {
            FeatherConfig.LOGGER.error("Failed to save config file!", e);
        }
    }
