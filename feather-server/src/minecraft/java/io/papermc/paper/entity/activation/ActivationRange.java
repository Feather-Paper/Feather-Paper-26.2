...
public final class ActivationRange {
    ...
    private static int getWakeUpDurationWithVariance(Entity entity, int wakeUpDuration) {
        ...
        final net.feathermc.feather.util.math.random.FasterRandomSource wakeUpDurationRandom = net.feathermc.feather.util.math.random.FasterRandomSource.SHARED_INSTANCE;
    ...
    public synchronized static void activateEntities(final Level world) { // Leaf - SparklyPaper - parallel world ticking
        ...
        for (final Player player : world.players()) {
            ...
            for (final Entity entity : entities) {
                ...
                // Pufferfish start
                if (net.feathermc.feather.config.modules.opt.DynamicActivationofBrain.enabled && entity.getType().dabEnabled &&
                        (!net.feathermc.feather.config.modules.opt.DynamicActivationofBrain.dontEnableIfInWater || entity.is(net.minecraft.tags.EntityTypeTags.CAN_BREATHE_UNDER_WATER) || !entity.isInWater())) { // Leaf - Option for dontEnableIfInWater
                    if (!entity.activatedPriorityReset) {
                        entity.activatedPriorityReset = true;
                        entity.activatedPriority = net.feathermc.feather.config.modules.opt.DynamicActivationofBrain.maximumActivationPrio;
                    }
                    int squaredDistance = (int) player.distanceToSqr(entity);
                    entity.activatedPriority = squaredDistance > net.feathermc.feather.config.modules.opt.DynamicActivationofBrain.startDistanceSquared ?
                            Math.max(1, Math.min(squaredDistance >> net.feathermc.feather.config.modules.opt.DynamicActivationofBrain.activationDistanceMod, entity.activatedPriority)) :
                            1;
                } else {
