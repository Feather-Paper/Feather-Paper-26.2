    ...
    static <B extends FriendlyByteBuf> StreamCodec<B, CustomPacketPayload> codec(
        ...
        return new StreamCodec<B, CustomPacketPayload>() {
            ...
            public void encode(final B output, final CustomPacketPayload value) {
                ...
                if (value instanceof net.feathermc.feather.protocol.LeafCustomPayload payload) {
                    net.feathermc.feather.protocol.Protocols.write(output, payload);
                    return;
                }
            ...
            public CustomPacketPayload decode(final B input) {
                ...
                return java.util.Objects.requireNonNullElseGet(payload, () -> this.findCodec(identifier).decode(input));
                // Leaves end - protocol core
            }
        };
    }
