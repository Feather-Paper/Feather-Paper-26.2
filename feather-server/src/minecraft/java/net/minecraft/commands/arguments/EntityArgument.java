...
public class EntityArgument implements ArgumentType<EntitySelector> {
    ...
    public static Collection<? extends Entity> getOptionalEntities(final CommandContext<CommandSourceStack> context, final String name) throws CommandSyntaxException {
        ...
        return net.feathermc.feather.config.modules.opt.SkipInactiveEntityForExecute.skipInactiveEntityForExecute && context.getSource().source instanceof net.minecraft.server.MinecraftServer ? entitySelector.findEntitiesSkipInactive(context.getSource()) : entitySelector.findEntities(context.getSource());
    ...
    public EntitySelector parse(final StringReader reader, final boolean allowSelectors, final boolean overridePermissions) throws CommandSyntaxException {
        // CraftBukkit end
        int start = 0;
        EntitySelectorParser parser = new EntitySelectorParser(reader, allowSelectors);
        EntitySelector selector = parser.parse(overridePermissions); // CraftBukkit
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(final CommandContext<S> contextBuilder, final SuggestionsBuilder builder) {
        if (contextBuilder.getSource() instanceof SharedSuggestionProvider source) {
            ...
            final boolean permission = source instanceof CommandSourceStack stack
                ...
                : source.permissions().hasPermission(Permissions.COMMANDS_ENTITY_SELECTORS);
            EntitySelectorParser parser = new EntitySelectorParser(reader, permission);
