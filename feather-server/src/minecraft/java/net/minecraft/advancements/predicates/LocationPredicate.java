...
public record LocationPredicate(
    ...
    public boolean matches(final ServerLevel level, final double x, final double y, final double z) {
        ...
        return (!this.biomes.isPresent() || loaded && this.biomes.get().contains(net.feathermc.feather.config.modules.opt.OptimizeBiome.advancement ? level.getBiomeCached(null, pos) : level.getBiome(pos))) // Leaf - cache getBiome
