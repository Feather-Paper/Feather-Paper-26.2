...
public interface RandomSource {
    ...
    static RandomSource createThreadSafe() {
        return net.feathermc.feather.config.modules.opt.FastRNG.enabled
                ? new net.feathermc.feather.util.math.random.FasterRandomSource(RandomSupport.generateUniqueSeed())
                : new ThreadSafeLegacyRandomSource(RandomSupport.generateUniqueSeed());
    }

    static RandomSource create(final long seed) {
        return net.feathermc.feather.config.modules.opt.FastRNG.enabled
                ? new net.feathermc.feather.util.math.random.FasterRandomSource(seed)
                : new LegacyRandomSource(seed);
    }

    static RandomSource createForSlimeChunk(final long seed) {
        return net.feathermc.feather.config.modules.opt.FastRNG.enabled && !net.feathermc.feather.config.modules.opt.FastRNG.useLegacyForSlimeChunk
                ? new net.feathermc.feather.util.math.random.FasterRandomSource(seed)
                : new SingleThreadedRandomSource(seed);
    }
    ...
    static RandomSource createLegacy(final long seed) {
