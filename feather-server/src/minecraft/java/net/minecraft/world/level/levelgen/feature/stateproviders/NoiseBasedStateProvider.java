...
public abstract class NoiseBasedStateProvider extends BlockStateProvider {
    ...
    protected NoiseBasedStateProvider(final long seed, final NormalNoise.NoiseParameters parameters, final float scale) {
        ...
        this.scale = scale;
        this.noise = NormalNoise.create(new WorldgenRandom(net.feathermc.feather.config.modules.opt.FastRNG.worldgenEnabled() ? new net.feathermc.feather.util.math.random.FasterRandomSource(seed) : new LegacyRandomSource(seed)), parameters); // Leaf - Faster random generator
    }
