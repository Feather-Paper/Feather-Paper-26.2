...
public class PerlinSimplexNoise {
    ...
    private PerlinSimplexNoise(final RandomSource random, final IntSortedSet octaveSet) {
        ...
        if (highFreqOctaves > 0) {
            ...
            RandomSource highFreqRandom = new WorldgenRandom(net.feathermc.feather.config.modules.opt.FastRNG.worldgenEnabled() ? new net.feathermc.feather.util.math.random.FasterRandomSource(positiveOctaveSeed) : new LegacyRandomSource(positiveOctaveSeed)); // Leaf - Faster random generator
