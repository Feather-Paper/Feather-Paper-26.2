...
public class OreFeature extends Feature<OreConfiguration> {
    ...
    protected boolean doPlace(
        ...
        int placed = 0;
        BitSet tested = net.feathermc.feather.util.cache.CachedOrNewBitsGetter.getCachedOrNewBitSet(sizeXZ * sizeY * sizeXZ); // Leaf - C2ME - Reduce Allocations
        ...
        for (int i = 0; i < size; i++) {
            ...
            double r = ((Mth.sin(Mth.PI * step) + 1.0F) * ss + 1.0) / 2.0;
            data[i * 4 + 0] = xx;
            data[i * 4 + 1] = yy;
            data[i * 4 + 2] = zz;
            data[i * 4 + 3] = r;
    ...
    public static boolean canPlaceOre(
        ...
        return targetState.target.test(orePosState, random)
            && (shouldSkipAirCheck(random, config.discardChanceOnAirExposure) || !isAdjacentToAir(blockGetter, orePos));
    }
