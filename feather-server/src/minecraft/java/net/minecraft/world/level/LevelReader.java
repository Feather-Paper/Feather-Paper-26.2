...
public interface LevelReader extends BlockAndLightGetter, CollisionGetter, SignalGetter, BiomeManager.NoiseBiomeSource, ca.spottedleaf.moonrise.patches.chunk_system.level.ChunkSystemLevelReader { // Paper - rewrite chunk system
    ...
    default @Nullable ChunkAccess getChunkIfLoadedImmediately(BlockPos pos) { return this.getChunkIfLoadedImmediately(pos.getX() >> 4, pos.getZ() >> 4); }
    ...
    default Holder<Biome> getBiomeCached(final net.minecraft.world.level.chunk.@Nullable LevelChunk chunk, final BlockPos pos) {
    ...
    default boolean hasChunksAt(final int x0, final int y0, final int z0, final int x1, final int y1, final int z1) {
        return y1 >= this.getMinY() && y0 <= this.getMaxY() && this.hasChunksAt(x0, z0, x1, z1);
    }
