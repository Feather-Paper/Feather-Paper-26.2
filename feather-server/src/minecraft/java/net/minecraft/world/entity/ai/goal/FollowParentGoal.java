...
public class FollowParentGoal extends Goal {
    ...
    private final int leaf$internal = Math.max(1, reducedTickDelay(net.feathermc.feather.config.modules.opt.EntityGoal.chanceFollowParent)); private int leaf$counter; // Leaf - configurable goal update interval
    @Override
    public boolean canUse() {
        leaf$counter++; if (leaf$counter < leaf$internal) { return false; } else { leaf$counter = this.animal.getRandom().nextInt(leaf$internal); } // Leaf - configurable goal update interval
