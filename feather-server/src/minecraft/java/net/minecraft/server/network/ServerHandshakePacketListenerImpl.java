...
public class ServerHandshakePacketListenerImpl implements ServerHandshakePacketListener {
    ...
    private void beginLogin(ClientIntentionPacket packet, final boolean transfer) { // Paper - remove packet final
        ...
        if (packet.protocolVersion() != SharedConstants.getCurrentVersion().protocolVersion()) {
            ...
            } else if (!net.feathermc.feather.config.modules.misc.RemoveSpigotCheckBungee.enabled && (split.length == 3 || split.length == 4) && (ServerHandshakePacketListenerImpl.HOST_PATTERN.matcher(split[1]).matches())) { // Leaf - Remove Spigot check for broken BungeeCord configurations
